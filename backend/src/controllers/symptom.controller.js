import SymptomTimeline from "../models/Symptom.model.js";
...
export const getTimeline = async (req, res) => {
  try {
    ...
    timeline.summary = {
      ...
      avgSeverity:
        symptomCount > 0 ? (totalSeverity / symptomCount).toFixed(1) : 0,
...
const generateRecommendations = (summary) => {
  ...
  if (summary.avgSeverity > 7) {
    recommendations.push(
      "High severity symptoms detected. Please consult a doctor immediately.",
    );
  }

  if (summary.mostFrequentSymptoms.includes("Fever")) {
    recommendations.push(
      "Monitor temperature regularly. Stay hydrated and rest.",
    );
  }
